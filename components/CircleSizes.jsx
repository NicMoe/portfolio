...
const units = [
  {
    unit: '$',
    phrase:
      'The red circle represents $VALUE - how much money does the blue circle represent?',
  },
  {
    unit: '€',
    phrase:
  ...
  },
  {
    unit: 'puppies',
    phrase:
      'The red circle represents VALUE puppies - how many puppies does the blue circle represent?',
  },
  {
    unit: 'apples',
    phrase:
      'The red circle represents VALUE apples - how many apples does the blue circle represent?',
  },
  {
    unit: 'cars',
    phrase:
      'The red circle represents VALUE cars - how many cars does the blue circle represent?',
  },
  {
    unit: 'houses',
    phrase:
      'The red circle represents VALUE houses - how many houses does the blue circle represent?',
  },
  {
    unit: 'pounds of sugar',
    phrase:
      'The red circle represents VALUE pounds of sugar - how many pounds of sugar does the blue circle represent?',
  },
  {
    unit: 'liters of vinegar',
    phrase:
      'The red circle represents VALUE liters of vinegar - how many liters of vinegar does the blue circle represent?',
  },
  {
    unit: 'books',
    phrase:
      'The red circle represents VALUE books - how many books does the blue circle represent?',
  },
  {
    unit: 'dogs',
    phrase:
      'The red circle represents VALUE dogs - how many dogs does the blue circle represent?',
  },
]
...
const InteractiveD3 = ({ postId }) => {
  ...
  const [redCircleValues, setRedCircleValues] = useState([
  ...
  const submitGuesses = async blueCircleValues => {
    try {
      // const response =
      await axios.post('/api/circle-size-guesses', {
        postId,
        guesses,
        circleData,
        redCircleValues,
    ...
    return (
      <div>
        ...
        {circleData.map((circles, index) => {
          const redArea = Math.PI * circles[0].r ** 2
          const blueArea = Math.PI * circles[1].r ** 2
          const relativeBlueValue = Math.round(
            (blueArea / redArea) * redCircleValues[index],
          )
          ...
          return (
            <div key={`result-${circles[0].x}-${circles[0].y}`}>
              ...
              <p>
                {randomPhrases[index].replace('VALUE', redCircleValues[index])}
              </p>
              <p>
                Your guess:{' '}
                {unit === '$' || unit === '€'
              ...
              <p>
                Correct size of blue circle:{' '}
                {unit === '$' || unit === '€'
                  ? `${unit}${relativeBlueValue}`
                  : `${relativeBlueValue} ${unit}`}
              </p>
            </div>
          )
        })}
  ...
  return (
    <div>
      <div>
        <svg ref={svgRef} />
        <p>
          {randomPhrases[currentChart].replace(
            'VALUE',
            redCircleValues[currentChart],
          )}
        </p>
        <input
          type="number"
